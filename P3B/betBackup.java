...
PVector calcColor(Hit hit, float d){
      ...
      col.z = diff.z + spec.z + amb.z + refl.z;
          ...
          //      Hit hitI = hit;
          //for (Light light : arrL) {
          //    l = (PVector.sub(light.origin, hitI.pt)).normalize();
          //    //max(0, hit normal . l)
          //    float maxo = max(0, (PVector.dot(l, hitI.normal)));
          //    col.x += light.r * maxo;
          //    col.y += light.g * maxo;
          //    col.z += light.b * maxo;
          //}
          //col.x = col.x * hitI.surface.Cdr;
          //col.y = col.y * hitI.surface.Cdg;
          //col.z = col.z * hitI.surface.Cdb;
      return col;
              ...
              if(body_t >= 0) {
                  ...
                  if(cyHit <= (cone.y + cone.h) && cyHit > cone.y){
                      ...
                      if(axisDist <= cone.radius) {
                          ...
                          //find the normal
                          //PVector t1 = PVector.sub(hitPoint, cap_cent);
                          ////PVector t1 = new PVector()
                          //PVector t2 = PVector.sub(base, hitPoint);
                          
                          //PVector cross = new PVector(t1.x * t2.x, t1.y * t2.y, t1.z * t2.z);
                          //float mag = sqrt(pow(cross.x, 2.0) + pow(cross.y, 2.0) + pow(cross.z, 2.0));
                          //PVector n = new PVector(cross.x/mag, cross.y/mag, cross.z/mag);
